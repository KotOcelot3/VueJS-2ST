...
Vue.component('column_1', {
    ...
    methods: {
        TaskCompleted(ColumnCard, task) {
            ...
            let length = 0
            for (let i = 0; i < 5; i++) {
                if (card.task[i].points != null) {
                    length++
                }
                for (let i = 0; i < 5; i++) {
                    if (card.status[i].completed === true) {
                        card.status++
                    }
                    
                  
            // console.log("Проверочка" + ColumnCard.status)
             if (card.status / length * 100 >= 50 && card.status / length * 100 < 100) {
                // console.log("1" + ColumnCard.status)
                eventBus.$emit('.addColumn_2', ColumnCard) 
                    } else if (this.column_2 === 5) {
                        this.errors.push('Завершите текущие задачи')
